...
func resourceNetworkingTrunkV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Waiting for OpenStack Neutron trunk (%s) to become available.", trunk)
	...
	if len(tags) > 0 {
		...
		log.Printf("[INFO] Set Tags = %+v on trunk %s", tags, trunk.ID)
	}
...
func resourceNetworkingTrunkV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("sub_port") {
		...
		oldSubportsMap := resourceNetworkingTrunkV2SubportsMap(old.(*schema.Set))
		newSubportsMap := resourceNetworkingTrunkV2SubportsMap(new.(*schema.Set))

		for oldSubport := range oldSubportsMap {
			if _, found := newSubportsMap[oldSubport]; !found {
				subportsToRemove.Subports = append(subportsToRemove.Subports, trunks.RemoveSubport{PortID: oldSubport.PortID})
			}
		}

		for newSubport := range newSubportsMap {
			if _, found := oldSubportsMap[newSubport]; !found {
				subportsToAdd.Subports = append(subportsToAdd.Subports, newSubport)
			}
		}

		if len(subportsToRemove.Subports) > 0 {
			log.Printf("[DEBUG] Updating trunk %s removing subports: %+v", d.Id(), subportsToRemove)
		...
		if len(subportsToAdd.Subports) > 0 {
			log.Printf("[DEBUG] Updating trunk %s adding subports: %+v", d.Id(), subportsToAdd)
		...
		log.Printf("[INFO] Updated subports = %+v on trunk %s", new.(*schema.Set).List(), d.Id())
	}
	...
	if d.HasChange("tags") {
		...
		log.Printf("[INFO] Updated tags = %+v on trunk %s", tags, d.Id())
	}
...
func resourceNetworkingTrunkV2SubportsMap(d *schema.Set) (subports map[trunks.Subport]bool) {
	rawSubports := d.List()

	subports = make(map[trunks.Subport]bool)
	for _, raw := range rawSubports {
