		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2NetworkExists("openstack_networking_network_v2.network_1", &network_1),
					testAccCheckNetworkingV2SubnetExists("openstack_networking_subnet_v2.subnet_1", &subnet_1),
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2PortExists("openstack_networking_port_v2.parent_port_1", &parent_port_1),
					testAccCheckNetworkingV2TrunkExists("openstack_networking_trunk_v2.trunk_1", []string{}, &trunk_1),
					testAccCheckNetworkingV2Tags("openstack_networking_trunk_v2.trunk_1", "tags", []string{"a", "b", "c"}),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2PortExists("openstack_networking_port_v2.parent_port_1", &parent_port_1),
					testAccCheckNetworkingV2TrunkExists("openstack_networking_trunk_v2.trunk_1", []string{}, &trunk_1),
					testAccCheckNetworkingV2Tags("openstack_networking_trunk_v2.trunk_1", "tags", []string{"c", "d", "e"}),
				),
			},
		},
	})
		...
		if len(subports) > 0 {
			...
			for i, subport := range subports {
				if subportResource, ok := s.RootModule().Resources[subportResourceNames[i]]; ok {
					...
					return fmt.Errorf("Subport not found: %s", subport)
				}
			}
		}
