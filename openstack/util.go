...
func testAccCheckNetworkingV2Tags(name string, key string, tags []string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		var tagLen int64
		var err error
		if count, ok := rs.Primary.Attributes[fmt.Sprintf("%s.#", key)]; !ok {
			return fmt.Errorf("resource tags not found: %s.%s", name, key)
		} else {
		...
		itags := flatmap.Expand(rs.Primary.Attributes, key).([]interface{})
		...
		if !reflect.DeepEqual(rtags, tags) {
			return fmt.Errorf(
				"%s.%s: expected: %#v, got %#v", name, key, tags, rtags)
		}
		return nil
	}
}
